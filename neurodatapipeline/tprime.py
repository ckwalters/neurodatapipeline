...
def get_spikes_kilosort(sorter_path: Path):
    ...
    print("Converted npy spike times to .txt file")
...
def tprime_command(session_path: Path, event_bits=[1,2], sorter="kilosort"):
    ...
    ni_events_paths = [
        find_file(session_path, key=f"nidq.xd_8_{event_bit}_0.txt") for event_bit in event_bits
    ]
    ni_events = []
    ...
    # addtl_probe_streams = ""
    # addtl_probe_events = ""
    # for probe_idx,probe_sync_path in enumerate(imec_dirs):
    #     if probe_idx==0: # first (original) probe, already done
    #         continue
    #     if sorter=="kilosort":
    #         imec_spiketimes = get_spikes_kilosort(probe_paths[probe_idx] / sorter)
    #     stream_index = probe_idx+1 # stream 1 is reserved for NI
    #     out_file = f"{probe_sync_path[0:-4]}_spiketimes_synced.txt"
    #     addtl_probe_streams += f"-fromstream={stream_index},{probe_sync_path}"
    #     addtl_probe_events += f"-events={stream_index},{imec_spiketimes},{out_file}"
    
    # Format command
    command = f"TPrime -syncperiod=1.0 -tostream={imec0_sync_path} -fromstream=1,{ni_sync_path} {' '.join(ni_events)}"
