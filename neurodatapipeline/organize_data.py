...
def organize_existing_sessions(search_str: str = ""):
    ...
    session_dirs = [
        d for d in SESSIONS_PATH.iterdir() if d.is_dir() and search_str in d.name
    ]
...
def organize_recording_session(sglx_session_path: Path, suppress_data_move=False):
    ...
    ## Ephys data
    import_sglx_data(
        session_dir, sglx_session_path, suppress_data_move=suppress_data_move
    )

    ## Behavior data
    import_behavior_data(
        session_dir,
        recording_datetime,
        subject_id,
        suppress_data_move=suppress_data_move,
    )
...
def maintain_recording_session(
    session_dir: Path,
    behavior_source=BEHAVIOR_PATH,
    ...
    rec_datetime = datetime.strptime(
        "_".join(session_name.split("_")[1:]), SESSION_DT_PATTERN
    )
...
def import_sglx_data(
    session_dir: Path, sglx_session_path: Path, suppress_data_move=False
):
    """
...
def find_sglx_recordings(search_str: str = "", source_path=SGLXDATA_PATH) -> list:
    ...
    possible_dirs = [
        d for d in source_path.iterdir() if d.is_dir() and search_str in d.name
    ]
    imec_dirs = []
...
def import_behavior_data(
    ...
    behavior_src_path = find_behavior_file(
        "behavior", rec_datetime, subject_id, source_path=source_path
    )
...
def import_video_data(
    ...
    video_src_path = find_behavior_file(
        "video", rec_datetime, subject_id, source_path=source_path
    )
