...
def find_channelmap(recording_path: Path) -> Path:
    ...
    channel_map_files = [
        f for f in recording_path.iterdir() if "kilosort_channel_map" in f.name
    ]
...
def prep_run_kilosort(recording_dir: Path, do_CAR=True, do_drift_correction=True):
    """
    ...
    # Run kilosort
    run_kilosort(
        settings=settings, probe=probe_params, do_CAR=do_CAR, results_dir=results_dir
    )
