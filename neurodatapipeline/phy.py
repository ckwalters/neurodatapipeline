...
def main():
    ...
    session_names = find_sessions(search_dict={"kilosort_bool":True, "phy_bool":False})
...
def run_phy(session_search_key: str, probe: int, session_source: Path=SESSIONS_PATH, sorting_name: str=None, force_waveforms=False):
    """
    ...
    # Find session
    session_dirs = [d for d in session_source.iterdir() if (session_search_key in d.name) and (d.is_dir())]
    if len(session_dirs)==0:
        ...
        return
    session_recording_dir = [d for d in (session_dirs[0] / "ephys_catgt").iterdir() if d.is_dir()][0]
    recording_path = [d for d in session_recording_dir.iterdir() if (f"imec{probe}" in d.name) and (d.is_dir())][0]  # type: Path
