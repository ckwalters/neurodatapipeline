...
def pipeline(session_path: Path, nodes=None):
    ...
    if recording_dir is None:
        log(
            "No recording session folder found in ephys_catgt dir",
            p=session_path.name,
            n="pipeline",
            c=13,
            w=True,
...
def node_kilosort(recording_dir: Path):
    ...
    ks_command = (
        f"conda run -n {KS_ENV} python {str(ks_script_path)} {str(recording_dir)}"
    )
    ks_status = subprocess.run(
        ks_command, capture_output=False, text=True, shell=True, errors="replace"
    )
...
def node_sortinganalyzer(ks_dir: Path):
    ...
    sa_command = (
        f"conda run -n {SI_ENV} python {str(sa_script_path)} {str(ks_dir.parent)}"
    )
    sa_status = subprocess.run(
        [part.strip() for part in sa_command.split(" ")], capture_output=True, text=True
    )
    if sa_status.returncode == 0:
        log(
            "Sorting analyzer completed",
            p=session_name,
            r=ks_dir.name,
            c=70,
            n="sortinganalyzer",
