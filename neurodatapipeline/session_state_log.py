...
def find_sessions(search_dict={}, mode="all", update=False):
    ...
    if not search_dict:
        keys = ["catgt_bool", "tprime_bool", "kilosort_bool",
                "phy_bool", "sa_bool", "lfp_bool", "lfp_dfs_bool"]
...
def search_sessions(search_all=True, search_key="", nodes=None, mode="all",
                    source_dir=SESSIONS_PATH):
    ...
    session_names = find_sessions(search_dict=dict(zip(search_keys,[False]*len(search_keys))),
                                  mode=mode)
