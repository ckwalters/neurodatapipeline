...
class SuperAnalyzer:
    def __init__(self, recording_path: Path, sorting_str: str=None, analyzer_name: str="sorting_analyzer",
                 make_plots=False):
        ...
        if sorting_str is None:
            # Most recent sorting
            sorter_dir = [dir for dir in os.listdir(self.recording_path) if "kilosort4" in dir][-1] 
        else:
    ...
    def make_sorting_analyzer(self, apply_CMR=False):
        ...
        # Load recording
        recording = se.read_spikeglx(self.recording_path, stream_id=f"imec{self.probe}.ap")
        ...
        sqm.compute_quality_metrics(sorting_analyzer, metric_names=["firing_rate","amplitude_median","snr","isi_violation"])
    ...
    def format_unit_metrics(self):
        ...
        unit_ids = pd.Series(self.sorting_analyzer.sorting.get_unit_ids(), index=unit_index, name="unit_id")
        shanks = pd.Series(self.sorting_analyzer.sorting.get_property("sh"), index=unit_index, name="shank")
        channels = pd.Series(self.sorting_analyzer.sorting.get_property("ch"), index=unit_index, name="channel")
        depths = pd.Series(self.sorting_analyzer.sorting.get_property("depth"), index=unit_index, name="depth")
        
        # Quality metrics
        quality_metrics = self.sorting_analyzer.get_extension("quality_metrics").get_data()
        
        # Template metrics
        template_metrics = self.sorting_analyzer.get_extension("template_metrics").get_data()
        
        # Kilosort label (inherited from cluster_info.tsv)
        kilosort_label = pd.Series(self.sorting_analyzer.sorting.get_property("KSLabel"), index=unit_index, name="ks_label")

        # Make dataframe
        self.unit_info = pd.concat([unit_ids, shanks, channels, depths, quality_metrics, template_metrics,
                                    kilosort_label], axis=1)
        ...
        for (unit_a, unit_b) in redundant_pairs:
            self.unit_info.loc[self.unit_info["unit_id"]==unit_a,"duplicate_id"] = unit_b
            self.unit_info.loc[self.unit_info["unit_id"]==unit_b,"duplicate_id"] = unit_a
    ...
    def curate_units(self, phy=PHY_CURATION, auto=AUTO_CURATION):
        ...
        if phy:
            unit_index = self.sorting_analyzer.sorting.get_unit_ids()
            phy_label = pd.Series(self.sorting_analyzer.sorting.get_property("quality"), index=unit_index, name="phy_label")
        ...
        if "phy_label" in self.unit_info.columns:
            self.good_units = self.unit_info.loc[self.unit_info["phy_label"]=="good", "unit_id"].to_list()
    ...
    def plot_unit_summary(self, unit, plot_all_clusters=True, export=False,
                          unit_color="k", good_color="mediumturquoise"):
        ...
        sw.plot_unit_waveforms(self.sorting_analyzer, unit_ids=[unit], same_axis=True, ax=ax,
                               unit_colors={unit: "gray"}, alpha_waveforms=0.2,
                               lw_templates=1, plot_legend=False)
        ...
        sw.plot_autocorrelograms(self.sorting_analyzer, unit_ids=[unit], unit_colors={unit: "gray"}, ax=ax)
        ...
        if plot_all_clusters:
            bad_units = self.unit_info.loc[((self.unit_info.index!=unit) &
                                            (self.unit_info.phy_label!="good"))].index
            good_units = self.unit_info.loc[((self.unit_info.index!=unit) & 
                                            (self.unit_info.phy_label=="good"))].index
            for unit_list,color in zip([bad_units,good_units], ["gray",good_color]):
                sw.plot_unit_locations(self.sorting_analyzer, unit_ids=unit_list, ax=ax,
                                    unit_colors=dict(zip(unit_list,[color]*len(unit_list))), plot_all_units=False)
            sw.plot_unit_locations(self.sorting_analyzer, unit_ids=[unit], ax=ax,
                                unit_colors={unit: unit_color}, plot_all_units=False)
        else:
            sw.plot_unit_locations(self.sorting_analyzer, unit_ids=[unit], unit_colors={unit: "k"}, ax=ax)
        ...
        if plot_all_clusters:
            ax.scatter(self.unit_info.loc[self.unit_info.phy_label!="good", "firing_rate"],
                       self.unit_info.loc[self.unit_info.phy_label!="good", "peak_to_valley"]*1000,
                       10, c="gray", alpha=0.1)
            ax.scatter(self.unit_info.loc[self.unit_info.phy_label=="good", "firing_rate"],
                       self.unit_info.loc[self.unit_info.phy_label=="good", "peak_to_valley"]*1000,
                       10, c="darkturquoise", alpha=0.5)
        ax.scatter(self.unit_info.loc[unit, "firing_rate"],
                   self.unit_info.loc[unit,"peak_to_valley"]*1000, 20, c="k")
        ax.set_ylim(0,2)
        ...
        if plot_all_clusters:
            ...
            sw.plot_amplitudes(self.sorting_analyzer, unit_ids=[unit], ax=ax, unit_colors={unit: "gray"}, plot_legend=False)
        ...
        fig.suptitle(f"Probe {self.probe} Unit {unit_name}    {self.recording_path.name}", fontsize=11)
        fig.tight_layout()

        # Save
        if export:
            fig.savefig(self.sorting_path / "qc_figures" / f"{self.recording_path.name}_{unit_name}", dpi=300)
            plt.close()
        else:
            plt.show()
    ...
    def plot_amplitudes(self, ax: plt.Axes, unit: int, unit_color, good_color):
        ...
        neighbor_chs = np.where(((np.abs(unit_x - channel_coords[:,0])<=15) & 
                                    (np.abs(unit_y - channel_coords[:,1])<=15)))[0]

        # Get same-channel units
        bad_units = self.unit_info.loc[((self.unit_info.channel.isin(neighbor_chs)) &
                                            (self.unit_info.index!=unit) &
                                            (self.unit_info.phy_label!="good"))].index
        good_units = self.unit_info.loc[((self.unit_info.channel==self.unit_info.channel.loc[unit]) &
                                            (self.unit_info.index!=unit) &
                                            (self.unit_info.phy_label=="good"))].index

        # Plot all background units
        for unit_list,color in zip([bad_units,good_units], ["gray",good_color]):
            sw.plot_amplitudes(self.sorting_analyzer,
                                unit_ids=unit_list,
                                ax=ax,
                                scatter_decimate=10,
        ...
        # Plot selected unit
        sw.plot_amplitudes(self.sorting_analyzer, unit_ids=[unit], ax=ax,
                           unit_colors={unit: unit_color}, plot_legend=False)
    ...
    def plot_waveforms(self, unit_id, ax, linecolor="gray", linealpha=0.05):
        ...
        best_channel = si.get_template_extremum_channel(self.sorting_analyzer, outputs="index")[unit_id]
        ...
        best_wfs = wfs[:,:,best_channel_idx].transpose() # rows: samples, cols: waveforms (105, 500)

        # Get median waveform
        median_wf = np.median(best_wfs, axis=1)

        # Convert samples to milliseconds ms_before=1.0, ms_after=1.5)
        wfs_ms_window = wfs.shape[1] / self.sorting_analyzer.recording.sampling_frequency*1000
        if np.isclose(wfs_ms_window, 2.5):
            times = np.linspace(-1.0, 1.5, best_wfs.shape[0]) # times in miliseconds
        else:
            print(f"Waveform window not 2.5 ms, plotting aligned to beginning of window.")
            times = np.linspace(0, wfs_ms_window, best_wfs.shape[0]) # times in miliseconds
